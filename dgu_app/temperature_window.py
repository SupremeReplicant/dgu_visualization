...
class ComparisonTableWindow(tk.Toplevel):
    def __init__(self, parent, table_data, check_results):
...
class TemperatureWindow(BaseWindow):
    def __init__(self, groups, group_names, temperature_data, full_temperature_data, parent=None):
        ...
        self.text_actors = []
        self.temp_text_actors = []

    def show(self):
        if self.temperature_data is None:
            print("Нет данных о температуре для отображения.")
            return
        ...
        def fill_color(start_idx, end_idx, start_color, end_color):
    ...
    def _create_initial_interface(self):
        """Создает начальные элементы интерфейса"""
        self._recreate_interface()
    
    def _recreate_interface(self):
        """Пересоздает все элементы интерфейса при изменении размера"""
