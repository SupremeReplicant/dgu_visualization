...
from .validation import check_temperatures, create_comparison_table
...
def detect_crosshairs(images_dir, model_path="models/crosshair_detector.pth"):
    """
    Обнаруживает перекрестия на изображениях без извлечения температуры.
    ...
    Возвращает:
        bool: True если все 29 перекрестий найдены, False если хотя бы одно не найдено
    """
    ...
    if len(image_files) < 29:
        raise ValueError(f"Найдено только {len(image_files)} изображений, требуется 29")
    
    image_files = sorted(image_files)[:29]
...
class MainMenu:
    ...
    def update_button_states(self):
        """Обновляет состояние кнопок в зависимости от доступности данных"""
    ...
    def load_temperature_data(self):
        """Загружает температурные данные из файла"""
    ...
    def process_photos_for_temperatures(self):
        """Обрабатывает фотографии для получения температурных данных"""
    ...
    def handle_temperature_error(self, error_message):
        """Обрабатывает ошибки загрузки температурных данных"""
    ...
    def run_temperature_window(self):
