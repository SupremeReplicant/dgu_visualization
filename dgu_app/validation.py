...
reference_limits: Dict[Any, float] = {
    1: 87,
    2: 51,
    3: 60,
    '4-15': 450,
...
def check_temperatures(temps: np.ndarray) -> Tuple[Dict[Any, bool], float]:
    if len(temps) < 29:
        logger.error("Temperature data must have at least 29 values")
    ...
    if len(segment_4_15) > 0:
        ...
        min_temp = np.min(segment_4_15)
        max_temp = np.max(segment_4_15)
        mean_4_15 = np.mean(segment_4_15)
...
def create_comparison_table(temps: np.ndarray, check_results: Dict[Any, Any], part_names: List[str]) -> List[Dict[str, Any]]:
